...
alz <- filter(finalData, ALZ == 1)

ggplot(alz, aes(x = utilizer_category_pre, fill = treatment)) +
  geom_histogram(stat = "count", position = 'dodge')

ord <- polr(utilizer_category_post ~ utilizer_category_pre + treatment, 
            data = alz)
...
Additionally, all of the difference in difference results were negative numbers indicating the treatment (daily aspirin dosing) actually increased the health care utilization of the treatment group.  All of the increases were relatively small, ~$1.00 per member per month.  
